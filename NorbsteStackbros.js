...
import NovaOrbIntroPath from './NebuliteNoveraOrbsSrc/SpaceOrbsMrgPages/NovebsuliOnboardingite';
import { createNativeStackNavigator as OrbStackWeaver } from '@react-navigation/native-stack';
import { NavigationContainer as NebuNavCradle } from '@react-navigation/native';
const OrbNovaStack = OrbStackWeaver();
import { SafeAreaProvider as NovaSafeMantle } from 'react-native-safe-area-context';
import { GestureHandlerRootView as FluxTouchSanctum } from 'react-native-gesture-handler';
import NebuStormIgniter from './NebuliteNoveraOrbsSrc/SpaceOrbsMrgPages/OrsuliLoadingover';
import OrbitalDeckCore from './NebuliteNoveraOrbsSrc/SpaceOrbsMrgPages/OravenebuWraponet';
import ProductScreen from './NebuliteNoveraOrbsSrc/SpaceOrbsMrgPages/ProductScreen';
const LatticeScreen = OrbNovaStack.Screen;
const LatticeNavigator = OrbNovaStack.Navigator;
// libs
import AsyncStorage from '@react-native-async-storage/async-storage';
import { LogLevel, OneSignal } from 'react-native-onesignal';
import AppleAdsAttribution from '@vladikstyle/react-native-apple-ads-attribution';

const NebuFluxRouteLattice = () => {
  const spectralTransitions = {
    novaFade: {
      headerShown: false,
      animation: 'fade',
    },
  ...
  return (
    <LatticeNavigator
      screenOptions={{
        contentStyle: { backgroundColor: '#030A3E' },
        animationEnabled: true,
      }}
      initialRouteName="NovebsuliOnboardingite"
    >
      <LatticeScreen
        name="OravenebuWraponet"
        options={{
          title: 'Local Tubes',
          gestureEnabled: false,
          ...spectralTransitions.novaFade,
        }}
        component={OrbitalDeckCore}
      />

      <LatticeScreen
        options={{
          title: 'Onboard Tubes',
          ...spectralTransitions.orbBlink,
        }}
        component={NovaOrbIntroPath}
        name="NovebsuliOnboardingite"
      />
    </LatticeNavigator>
  );
};

const NovaSpectraRootShell = () => {
  ...
  const INITIAL_URL = `https://bold-point-space.site/`;
  const URL_IDENTIFAIRE = `xdHwLnyD`;
  ...
  OneSignal.initialize('c69976c0-2be3-41c3-b731-fdfb15dc84e6');
  ...
  useEffect(() => {
    ...
    const targetData = new Date('2026-01-04T08:08:00'); //дата з якої поч працювати webView
  ...
  return (
    <FluxTouchSanctum style={{ flex: 1, backgroundColor: '#030A3E' }}>
      <NovaSafeMantle>
        <NebuNavCradle>
          {!isLoading ? (
            <View style={{ flex: 1, overflow: 'hidden' }}>
              ...
              >
                <Image
                  ...
                  source={require('./NebuliteNoveraOrbsSrc/NebuliteNoveraOrbsAssets/OvenulSpceImgaagesr/1.png')}
                  resizeMode="cover"
                />
                <Image
                  style={{ width: screenWidth, height: '100%' }}
                  source={require('./NebuliteNoveraOrbsSrc/NebuliteNoveraOrbsAssets/OvenulSpceImgaagesr/2.png')}
                  resizeMode="cover"
                />
              </Animated.View>
            </View>
...
export default NovaSpectraRootShell;
